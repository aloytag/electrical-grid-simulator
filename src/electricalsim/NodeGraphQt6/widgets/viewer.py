...
from NodeGraphQt6.base.menu import BaseMenu
...
from NodeGraphQt6.qgraphics.node_abstract import AbstractNodeItem
from NodeGraphQt6.qgraphics.node_backdrop import BackdropNodeItem
from NodeGraphQt6.qgraphics.pipe import PipeItem, LivePipeItem
from NodeGraphQt6.qgraphics.port import PortItem
from NodeGraphQt6.qgraphics.slicer import SlicerPipeItem
from NodeGraphQt6.widgets.dialogs import BaseDialog, FileDialog
from NodeGraphQt6.widgets.scene import NodeScene
from NodeGraphQt6.widgets.tab_search import TabSearchMenuWidget
...
class NodeViewer(QtWidgets.QGraphicsView):
    ...
    class:`NodeGraphQt6.NodeGraph` class.
    """
    ...
    def _on_search_submitted(self, node_type):
        ...
        """
        pos = self.mapToScene(self._previous_pos.toPoint())
    ...
    def mousePressEvent(self, event):
        ...
        self._origin_pos = event.position()
        self._previous_pos = event.position()
        (self._prev_selection_nodes,
        ...
        if self.LMB_state and not items:
            rect = QtCore.QRect(self._previous_pos.toPoint(), QtCore.QSize())
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self._rubber_band.isActive:
            ...
            if self._rubber_band.isVisible():
                ...
                rect = QtCore.QRect(self._origin_pos.toPoint(), event.position().toPoint()).normalized()
        ...
        if moved_nodes and not self.COLLIDING_state:
            self.moved_nodes.emit({n.id: xy_pos for n, xy_pos in moved_nodes.items()})
    ...
    def mouseMoveEvent(self, event):
        if self.ALT_state and self.SHIFT_state:
            if self.pipe_slicing:
                if self.LMB_state and self._SLICER_PIPE.isVisible():
                    ...
                    p2 = self.mapToScene(self._previous_pos.toPoint())
        ...
        if self.MMB_state and self.ALT_state:
            ...
            self._set_viewer_zoom(zoom, 0.05, pos=event.position())
        elif self.MMB_state or (self.LMB_state and self.ALT_state):
            previous_pos = self.mapToScene(self._previous_pos.toPoint())
        ...
        if self.LMB_state and self._rubber_band.isActive:
            ...
            if max(rect.width(), rect.height()) > 5:
                ...
                self.scene().setSelectionArea(
                    path, mode=QtGui.Qt.ItemSelectionMode.IntersectsItemShape
                )
                self.scene().update(map_rect)
    ...
    def keyPressEvent(self, event):
        ...
        if overlay_text:
            ...
            self._cursor_text.setPos(self.mapToScene(self._previous_pos.toPoint()))
    ...
    def start_live_connection(self, selected_port):
        ...
        self._LIVE_PIPE.draw_index_pointer(
            selected_port,
            self.mapToScene(self._origin_pos.toPoint())
        )
    ...
    def scene_cursor_pos(self):
        ...
        """
        return self.mapToScene(self._previous_pos.toPoint())
